...
export default function Deals() {
  ...
  const createDealMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await apiRequest("POST", "/api/v1/deals", {
        ...data,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`${API_URL}/api/v1/deals`] });
  ...
  const updateDealMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      ...
      const res = await apiRequest("PATCH", `/api/v1/deals/${id}`, payload);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`${API_URL}/api/v1/deals`] });
  ...
  const deleteDealMutation = useMutation({
    mutationFn: async (id: number) => {
      return apiRequest("DELETE", `/api/v1/deals/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`${API_URL}/api/v1/deals`] });
